...
1. Go to the created directory of GeneAgent
   ...
   cd {directory}
   ```
2. Open the **.env copy** file and replace the API configuration with your own settings:
   ```
   OPENAI_API_KEY=YOUR_OWN_OPENAI_KEY
   OPENAI_BASE_URL=YOUR_OWN_OPENAI_BASE_SETTING
   ```

> [!TIP]
> Make sure to rename the file from **.env copy** to **.env** after updating the configuration. This centralized configuration will be used by all GeneAgent variants including **main_cascade.py**, **main_CoT.py**, **main_summary.py**, and **worker.py**.
